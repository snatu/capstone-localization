...
import sys
sys.path.append('/data/data/sherlock/training_code/BLIP')
...
class CLIPDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        inputs = self.tokenizer(c_data['inputs']['clue'], return_tensors="pt", padding='max_length', max_length=100)
        ...
        return {'image':image, 'caption':caption, 'clue': clue, 'id': cid, 'input_ids': input_ids, 'attention_mask': attention_mask, 'token_type_ids': token_type_ids}
...
def parse_args():
    ...
    parser.add_argument('train_context')
    parser.add_argument('val_context')
...
def prepare_image_inputs(cfg, img_list, model):
    ...
    images =  ImageList.from_tensors(images,model.module.backbone.size_divisibility)
    
    return images, batched_inputs

def get_features(model, images):
    features = model.module.backbone(images.tensor.cuda())
    return features

def get_proposals(model, images, features):
    proposals, _ = model.module.proposal_generator(images, features)
    return proposals

def get_box_features(model, features, proposals, batch_size):
    features_list = [features[f] for f in ['p2', 'p3', 'p4', 'p5']]
    box_features = model.module.roi_heads.box_pooler(features_list, [x.proposal_boxes for x in proposals])
    box_features = model.module.roi_heads.box_head.flatten(box_features)
    box_features = model.module.roi_heads.box_head.fc1(box_features)
    box_features = model.module.roi_heads.box_head.fc_relu1(box_features)
    box_features = model.module.roi_heads.box_head.fc2(box_features)

    box_features = box_features.reshape(batch_size, -1, 1024) # depends on your config and batch size
    return box_features, features_list

def get_prediction_logits(model, features_list, proposals):
    cls_features = model.module.roi_heads.box_pooler(features_list, [x.proposal_boxes for x in proposals])
    cls_features = model.module.roi_heads.box_head(cls_features)
    pred_class_logits, pred_proposal_deltas = model.module.roi_heads.box_predictor(cls_features)
...
def clip_forward(clip_model, image_model, image, text, input_ids, attention_mask, token_type_ids):
    image_features = image_model(image, input_ids, attention_mask, token_type_ids)
    # if len(image.shape) == 5:
    #     im_feat1 = image_model(image, bert)
    #     im_feat2 = image_model(image, bert)
    #     image_features = (im_feat1 + im_feat2) / 2
    # else:
    #     image_features = image_model(image, bert)
    #image_features should be batch_size x 512
...
def orig_clip_forward(model, blip_model, image, text, clue):
    # if len(image.shape) == 5:
    ...
    image_features = blip_model(image, clue)
...
class CLIPExtractor(torch.nn.Module):
    def __init__(self, clip_model, visbert_model, args):
        super(CLIPExtractor, self).__init__()
        self.clip_model = clip_model
        self.visbert_model = visbert_model
        self.args = args

    def forward(self, image, text, input_ids, attention_mask, token_type_ids):
        return clip_forward(self.clip_model, self.visbert_model, image, text, input_ids, attention_mask, token_type_ids)
    
class OrigCLIPExtractor(torch.nn.Module):
    def __init__(self, clip_model, blip_model, args):
        super(OrigCLIPExtractor, self).__init__()
    ...
    def forward(self, image, text, clue):
        return orig_clip_forward(self.clip_model, self.blip_model, image, text, clue)
...
def convert_image_to_bgr(images):
    images = list(images)
    converted_images = []
    for image in images:
        image = torch.movedim(image, 0, 2)
        converted_images.append(cv2.cvtColor(image.numpy(), cv2.COLOR_RGB2BGR))
    return converted_images

class VisBertModel(torch.nn.Module):

    def __init__(self):
        super(VisBertModel, self).__init__()
    ...
    def forward(self, x, input_ids, attention_mask, token_type_ids):
        visual_token_type_ids = torch.ones(x.shape[:-1], dtype=torch.long).cuda()
        visual_attention_mask = torch.ones(x.shape[:-1], dtype=torch.float).cuda()
        outputs = self.model(input_ids=input_ids.squeeze(1), attention_mask=attention_mask.squeeze(1), token_type_ids=token_type_ids.squeeze(1), visual_embeds=x, visual_attention_mask=visual_attention_mask, visual_token_type_ids=visual_token_type_ids)
        embedding = self.linear1(outputs.last_hidden_state)
        return embedding[:,-1, :]
...
class BLIPModel(torch.nn.Module):
    ...
    def forward(self, image, caption):
        outputs = self.model(image, caption, mode='multimodal')
        projection = self.linear1(outputs[:,0,:])
        return projection
...
def main():
    ...
    # object_model = get_model(cfg)
    # visbert_model = VisBertModel()
    ...
    if args.hide_true_bbox != 4:
        model = OrigCLIPExtractor(model, blip_model, args)
    else:
    ...
    for name, param in model.named_parameters():
        if not 'linear1' or 'text_projection' in name:
            param.requires_grad = False

    model.to(args.device)
    # blip_model = blip_model.to(args.device)

    # object_model.to(args.device)
    # visbert_model.to(args.device)
    ...
    with open(args.train_context) as f:
        for line in f:
            try:
                clue, context = line.split("\t")
                train_clue_context[clue] = context[9:]
            except:
                pass
    ...
    with open(args.train) as f:
        train = json.load(f)
        for input in train:
            clue = input['inputs']['clue']
            if clue not in train_clue_context:
                context = ""
            else: context = train_clue_context[clue]
            input['inputs']['knowledge'] = context
    ...
    val_clue_context = {}
    with open(args.val_context) as f:
        for line in f:
            try:
                clue, context = line.split("\t")
                val_clue_context[clue] = context[9:]
            except:
                pass
    ...
    with open(args.val) as f:
        ...
        for input in val:
            clue = input['inputs']['clue']
            if clue not in val_clue_context: context = ""
            else: context = val_clue_context[clue]
            input['inputs']['knowledge'] = context
    ...
    #     print('Lets use', torch.cuda.device_count(), 'GPUs!')
        ...
        # blip_model = torch.nn.DataParallel(blip_model)
        # object_model = torch.nn.DataParallel(object_model)
        # visbert_model = torch.nn.DataParallel(visbert_model)
        ...
        for mode in ['train', 'val']:
            ...
            for i, batch in bar:
                with torch.set_grad_enabled(mode=='train' and len(negative_sample_cache) == args.negative_sample_cache):
                    ...
                    if args.hide_true_bbox != 4:
                        images, captions, input_ids, attention_mask, token_type_ids = batch['image'].to(args.device), batch['caption'].to(args.device), batch['input_ids'].to(args.device), batch['attention_mask'].to(args.device), batch['token_type_ids'].to(args.device)
                        # images = convert_image_to_bgr(images.cpu())
                        # images, batched_inputs = prepare_image_inputs(cfg, images, object_model)
                        # features = get_features(object_model, images)
                        # proposals = get_proposals(object_model, images, features)
                        # batch_size = len(images)
                        # box_features, features_list = get_box_features(object_model, features, proposals, batch_size)
                        # pred_class_logits, pred_proposal_deltas = get_prediction_logits(object_model, features_list, proposals)
                        # boxes, scores, image_shapes = get_box_scores(cfg, pred_class_logits, pred_proposal_deltas, proposals)
                        # output_boxes = [get_output_boxes(boxes[i], batched_inputs[i], proposals[i].image_size) for i in range(len(proposals))]
                        # output_boxes, scores  = output_boxes, scores
                        ...
                        # image_features, text_features = model(visual_embeds, captions, input_ids, attention_mask, token_type_ids)
                        image_features, text_features = model(images, captions, list(batch['clue']))
                    else:
