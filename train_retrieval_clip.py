...
class CLIPDataset(torch.utils.data.Dataset):
    def __init__(self, data, args, training=False):
        self.args = args
        self.data = data
        self.id2data = {d['Flickr30kID']: d for d in self.data}
    ...
    def __getitem__(self, idx):
        ...
        #for now, look at hypothesis only, try hypothesis + explanation later?
        caption = clip.tokenize(c_data["hypothesis"], truncate=True).squeeze()  

        cid = c_data['Flickr30kID']
